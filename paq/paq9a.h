...
U32 MEM=1<<23;  // Global memory limit, 1 << 22+(memory option)
...
int get4(std::istream& in) {
  ...
  r = r * 256 + in.get();
  // char chs[4];
  // std::cout << "before get4 position " << in.tellg() << '\n'; 
  // in.read(&chs[0], 4);
  // std::cout << "chs " << (chs[0]&255) << " " << " " << (chs[1]&255) << " " << (chs[2]&255) << " " << (chs[3]&255) << '\n';
  // std::cout << "after get4 position " << in.tellg() << '\n'; 
  // r = ((chs[0]&255)<<24)+((chs[1]&255)<<16)+((chs[2]&255)<<8)+(chs[3]&255);
  // std::cout << "get4 " << r << '\n'; 
  return r;
}

void put4(U32 c, std::ostream& out) {
  out.put(c>>24);
  out.put(c>>16);
  out.put(c>>8);
  out.put(c);
  // out << U8(c>>24) << U8(c>>16) << U8(c>>8) << U8(c);
}
...
public:
  Encoder(Mode m, std::istream& in, std::ostream& out);
  ~Encoder() {
    if (buf) free(buf);
    buf = 0;
  }
  ...
  int code(int y=0) {
    ...
    while (((x1^x2)&0xff000000)==0) {  // pass equal leading bytes of range
      ...
      x1<<=8;
      x2=(x2<<8)+255;
      if (mode==DECOMPRESS) x=(x<<8)+in.get();
    }
    return y;
  }
...
Encoder::Encoder(Mode m, std::istream& sin, std::ostream& sout):
    mode(m), out(sout), in(sin), x1(0), x2(0xffffffff), x(0), 
...
void paq9a(std::istream& in, std::ostream& out, Mode mode) {
  if (!lzp && !predictor) {
    lzp=new LZP;
  ...
  if (mode==COMPRESS) {
    Encoder e(COMPRESS, emptyin, out);
    int c;
    while ((c=in.get())!=EOF) {
      int cp=lzp->c();
      if (c==cp)
    ...
    int usize=get4(in);
    get4(in);  // csize
    Encoder e(DECOMPRESS, in, emptyout);
    while (usize--) {
      int c=lzp->c();
      if (e.code()==0) {
        c=1;
...
std::string compress_string(std::string& in) {
  std::istringstream sin(in);
  std::ostringstream sout;
  paq9a(sin, sout, COMPRESS);
  reset();
  return sout.str();
}


std::string decompress_string(std::string &in) {
  std::istringstream sin(in);
  std::ostringstream sout;
  char ch;
  sin>>ch; // ch = 0;
  // std::cout << "should be 0 " << U32(ch&255) << '\n';
  // std::cout << "before while position " << sin.tellg() << '\n'; 
  while (sin>>ch) {
    // now ch = 'c';
    // std::cout << "should be c " << ch << '\n';
    // std::cout << "before position " << sin.tellg() << '\n'; 
    paq9a(sin, sout, DECOMPRESS);
    // std::cout << "after position " << sin.tellg() << '\n'; 
    sin>>ch;
  }
  reset();
  return sout.str();
